...
There are two ways you can trade with SiliconeTrader. Virtual trading is enabled on by default.  
...
You should now have the generated keys.bin file in your SiliconeTrader directory. This file contains your encrypted API keys and it is only valid for the current user and only on the computer it is created on.  
...
If enabled then Signal Trailing in SiliconeTrader is the first stage of buying where it will trail a coin based on the settings you specify for the time duration's you specify. If a coin matches the trailing conditions it then checks the conditions of the coin and buys the coin if these are met. 
...
This is the default screen that SiliconeTrader starts at. The table shows all the pairs that you currently hold. The data automatically refreshes every 5 seconds.
...
- Log Out: log out of SiliconeTrader
...
#### SiliconeTrader is not starting or crashing on startup
...
#### SiliconeTrader is not trading or is behaving weirdly in general
Again, check the log file for any errors, it is very likely that there will be a clue there.

License & Disclaimer
-------------

By using, or simply downloading SiliconeTrader (the Software), you understand and accept the following:
